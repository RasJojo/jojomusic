...
let MusicService = class MusicService {
    ...
    async podcastEpisodes(podcast, limit = 16) {
        ...
        return rawItems.slice(0, limit).map((item, index) => {
            ...
            return {
                episode_key: `${podcast.podcast_key}-${(0, payloads_1.normalizeValue)(item.guid ?? item.id ?? item.title ?? `${index}`)}`,
    ...
    hasUsefulIndexedSearch(response, limit) {
        if (!response) {
            return false;
        }
        return (response.artists.length > 0 ||
