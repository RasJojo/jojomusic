...
let MeService = class MeService {
    ...
    redisCache;
    homeCache = new Map();
    constructor(prisma, appConfig, musicService, authService, redisCache) {
        this.prisma = prisma;
    ...
    async getLikes(user) {
        ...
        const tracks = rows.map((row) => row.trackPayload);
        void this.musicService.primeAudioAssets(tracks, 8);
        return tracks;
    }
    async likeTrack(user, track) {
    ...
    async unlikeTrack(user, trackKey) {
    ...
    async getHistory(user) {
        ...
        return rows.map((row) => ({
            ...
            track_payload: row.trackPayload,
            created_at: row.createdAt,
        }));
    }
    async addHistory(user, payload) {
        ...
        return {
            ...
            track_payload: event.trackPayload,
            created_at: event.createdAt,
        };
    }
    async getFollowedPodcasts(user) {
        const rows = await this.prisma.savedPodcastShow.findMany({
            where: { userId: user.id },
            orderBy: { createdAt: 'desc' },
        });
        const podcasts = rows.map((row) => row.podcastPayload);
        return this.musicService.attachManagedPodcastArtwork(podcasts);
    }
    async followPodcast(user, podcast) {
    ...
    async unfollowPodcast(user, podcastKey) {
    ...
    async home(user) {
        ...
        const [likedRows, historyRows, recommendations, browseCategories, featuredPodcasts] = await Promise.all([
        ...
        const likedTracks = likedRows.map((row) => row.trackPayload);
        const recentlyPlayedMap = new Map();
        historyRows.forEach((row) => {
            if (!recentlyPlayedMap.has(row.trackKey)) {
                recentlyPlayedMap.set(row.trackKey, row.trackPayload);
            }
        });
        let recentlyPlayed = [...recentlyPlayedMap.values()].slice(0, 12);
        const generatedPlaylists = await this.safeTimed(() => this.buildGeneratedPlaylists(user, recommendations), [], 2500);
        const artistLookup = new Map();
        const [hydratedRecommendations, hydratedHistory, hydratedLikes] = await Promise.all([
            this.safeTimed(() => this.musicService.hydrateTrackVisuals(recommendations, artistLookup, null, 2), recommendations, 1200),
            this.safeTimed(() => this.musicService.hydrateTrackVisuals(recentlyPlayed, artistLookup, null, 0), recentlyPlayed, 800),
            this.safeTimed(() => this.musicService.hydrateTrackVisuals(likedTracks, artistLookup, null, 0), likedTracks, 800),
        ]);
        const [managedRecommendations, managedHistory, managedLikes, managedGeneratedPlaylists, managedBrowseCategories, managedFeaturedPodcasts,] = await Promise.all([
            this.safeTimed(() => this.musicService.attachManagedTrackVisuals(hydratedRecommendations), hydratedRecommendations, 1200),
            this.safeTimed(() => this.musicService.attachManagedTrackVisuals(hydratedHistory), hydratedHistory, 800),
            this.safeTimed(() => this.musicService.attachManagedTrackVisuals(hydratedLikes), hydratedLikes, 800),
            this.safeTimed(() => this.attachManagedGeneratedPlaylists(generatedPlaylists), generatedPlaylists, 1200),
            this.safeTimed(() => this.musicService.attachManagedBrowseArtwork(browseCategories), browseCategories, 800),
            this.safeTimed(() => this.musicService.attachManagedPodcastArtwork(featuredPodcasts), featuredPodcasts, 800),
        ]);
    ...
    getSpotifyConfigurationStatus() {
        ...
        return {
            ...
            configuration_hint: missing.length === 0 ? null : `Le serveur doit encore définir ${missing.join(', ')}.`,
        };
    }
    async spotifyStatus(user) {
        ...
        return {
            ...
            saved_shows: shows.map((row) => row.podcastPayload),
        };
    }
    spotifyConnect(user) {
        const config = this.getSpotifyConfigurationStatus();
        if (!config.configured) {
            throw new common_1.ServiceUnavailableException('Spotify integration is not configured');
        }
        const state = this.authService.createToken({ sub: user.id, kind: 'spotify_link' }, 15);
    ...
    async spotifySync(user) {
        ...
        if (!link) {
            throw new common_1.NotFoundException('Spotify account not linked');
        }
    ...
    async spotifyCallback(code, state, error) {
        if (error) {
            return this.spotifyCallbackHtml(false, `Spotify a refusé la connexion: ${error}.`);
        }
        const config = this.getSpotifyConfigurationStatus();
        if (!config.configured) {
            return this.spotifyCallbackHtml(false, "Le backend JojoMusique n'est pas configuré pour Spotify.");
        }
        if (!code || !state) {
        ...
        let payload;
        try {
            payload = this.authService.decodeToken(state);
        }
        catch {
            return this.spotifyCallbackHtml(false, 'Le state Spotify est invalide ou expiré.');
        }
        if (payload.kind !== 'spotify_link' || !payload.sub) {
            return this.spotifyCallbackHtml(false, 'Le state Spotify ne correspond pas à une liaison valide.');
        }
        const user = await this.prisma.user.findUnique({ where: { id: payload.sub } });
        if (!user) {
            return this.spotifyCallbackHtml(false, "Le compte JojoMusique lié à cette importation est introuvable.");
        }
        try {
            ...
            if (existingSameSpotify) {
                return this.spotifyCallbackHtml(false, 'Ce compte Spotify est déjà lié à un autre compte JojoMusique.');
            }
            const link = (await this.prisma.spotifyAccountLink.findUnique({ where: { userId: user.id } })) ??
                {
                    id: (0, payloads_1.generateId)(),
                    userId: user.id,
                    spotifyUserId: `${bundle.profile.id}`,
            ...
            await this.importSpotifyBundle(user, link, bundle, tokens);
            return this.spotifyCallbackHtml(true, `Import terminé: ${bundle.likedTracks.length} titres likés, ${bundle.savedShows.length} shows et ${bundle.savedEpisodes.length} épisodes sauvegardés.`);
        }
        catch (errorObject) {
            return this.spotifyCallbackHtml(false, this.spotifyErrorMessage(errorObject));
        }
    }
    async buildRecommendations(user, limit) {
        ...
        historyRows.forEach((row) => {
            ...
            const delta = row.eventType === 'track_completed'
                ? 3
        ...
        const seedTracks = [
            ...likedRows.slice(0, 2).map((row) => row.trackPayload),
            ...historyRows
                .filter((row) => ['play_started', 'track_completed'].includes(row.eventType))
                .slice(0, 2)
                .map((row) => row.trackPayload),
        ];
        const similarBatches = await Promise.all(seedTracks.map((seed) => this.musicService
        ...
        const topArtists = [...artistWeights.entries()]
            ...
            .slice(0, 2)
            .map(([artist]) => artist);
        const artistBatches = await Promise.all(topArtists.map((artist) => this.musicService.topTracksForArtist(artist, 8).catch(() => [])));
    ...
    async buildGeneratedPlaylists(user, recommendationsSeed) {
        const [likedRows, historyRows] = await Promise.all([
            this.prisma.savedTrack.findMany({
                where: { userId: user.id },
                orderBy: { createdAt: 'desc' },
        ...
        const likedTracks = likedRows.map((row) => row.trackPayload);
        const historyTracks = historyRows.map((row) => row.trackPayload);
        const playlists = [];
        ...
        if (likedTracks.length > 0 || historyTracks.length > 0) {
            const rewind = this.dedupeTracks([...historyTracks.slice(0, 6), ...likedTracks.slice(0, 6)]).slice(0, 12);
            playlists.push({
    ...
    editorialTracks(excludedKeys, limit) {
        const queries = [
            'new music friday',
            'pop hits',
            'rap francais',
    ...
    bestTrackVisual(tracks) {
        return (tracks.find((track) => track.artwork_url)?.artwork_url ??
    ...
    async safeTimed(fn, fallback, timeoutMs) {
        try {
    ...
    async attachManagedGeneratedPlaylists(playlists) {
        const tracksByPlaylist = await Promise.all(playlists.map((playlist) => this.enrichGeneratedPlaylistTracks(playlist.tracks)));
        return playlists.map((playlist, index) => ({
            ...playlist,
            tracks: tracksByPlaylist[index] ?? playlist.tracks,
            artwork_url: this.bestTrackVisual(tracksByPlaylist[index] ?? playlist.tracks) ??
                playlist.artwork_url,
        }));
    }
    async enrichGeneratedPlaylistTracks(tracks) {
        const subset = await Promise.all(tracks.slice(0, 4).map((track) => this.enrichGeneratedTrackVisualFallback(track)));
    ...
    async enrichGeneratedTrackVisualFallback(track) {
    ...
    async refreshManagedHomeResponse(response) {
        const [recentlyPlayed, likedTracks, recommendations, generatedPlaylists, browseCategories, featuredPodcasts,] = await Promise.all([
    ...
    async ensureSpotifyAccessToken(link) {
        ...
        if (!link.refreshToken) {
            throw new common_1.ConflictException('Spotify refresh token missing');
        }
    ...
    async refreshSpotifyAccessToken(refreshToken) {
        const body = new URLSearchParams({
    ...
    async spotifyTokenRequest(body) {
        const basic = Buffer.from(`${this.appConfig.spotifyClientId}:${this.appConfig.spotifyClientSecret}`).toString('base64');
    ...
    async fetchSpotifyApi(path, accessToken, params) {
    ...
    async fetchSpotifyImportBundle(accessToken) {
    ...
    async fetchSpotifySavedTracks(accessToken) {
    ...
    async fetchSpotifySavedShows(accessToken) {
    ...
    async fetchSpotifySavedEpisodes(accessToken) {
    ...
    async fetchSpotifyRecentTracks(accessToken) {
    ...
    async spotifyPaginate(path, accessToken) {
        const items = [];
        let offset = 0;
        const limit = 50;
        while (true) {
    ...
    mapSpotifyEpisode(episode) {
        ...
        return {
            ...
            duration_seconds: typeof episode.duration_ms === 'number' ? Math.round(episode.duration_ms / 1000) : null,
    ...
    async importSpotifyBundle(user, link, bundle, tokens) {
        this.invalidateHomeCache(user.id);
        const avatar = Array.isArray(bundle.profile.images) && bundle.profile.images[0]?.url
    ...
    spotifyErrorMessage(errorObject) {
        const message = String(errorObject);
        if (message.startsWith('403:')) {
            return ('Spotify a bien autorisé la connexion, mais la Web API refuse encore ce compte. ' +
    ...
    spotifyCallbackHtml(success, message) {
